...
void *PHILOSOPHER (void *arg){
	...
	if (eat) {
            ...
            STRING (P_STRING);
        //-------------SELECTING LEFT FORK BASED ON ITS AVAILABILITY--------------------
	    LEFT_FORK = (PHILOSOPHER_NUM == 4) ? 0 : PHILOSOPHER_NUM;
            if ((r = pthread_mutex_lock (&fork_mutex [LEFT_FORK])) != 0) {
                printf("error");exit(EXIT_FAILURE);
            }
        //-------------SELECTING RIGHT FORK BASED ON ITS AVAILABILITY--------------------
	    RIGHT_FORK = (PHILOSOPHER_NUM == 4) ? 4 : PHILOSOPHER_NUM + 1;
            ...
            //------------------CHECKING IF FORKS ARE FREE OR NOT --------------------------
...
void *TEMP_FUNCTION (void *arg){
    ...
    while (1) {  
        ...
        //----------------UNLOCK MUTEX FOR ACCESS REQUEST--------------------
        if ((r = pthread_mutex_unlock (&BUFFER_MUTEX)) != 0) {
            printf("error");exit(EXIT_FAILURE);
        }
...
int main (int argc, char **argv){
    ...
    while (LINE) sleep (1);
    //-----------------------IF NO MORE LINE ARE REQUIRED TO PRINT-----------------------
